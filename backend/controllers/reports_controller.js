...
exports.getEvaluationResult = async (req, res, next) => {
    try {
        ...
        if (assignments.length === 0) {
            return res.status(200).json({ status: 'success', message: 'ดึงข้อมูลสำเร็จ', data: { results: [] } });
        }
        ...
        for (const assignment of assignments) {
            ...
            if (assignment.results.length < totalIndicatorsCount) {
                if (userRole === 'EVALUATEE') {
                    return next(new AppError('การประเมินยังไม่เสร็จสมบูรณ์', 403));
                } else {
                    ...
                    finalResults.push({
                        ...
                        status: 'INCOMPLETE',
                        message: 'รอผู้ประเมินให้คะแนนให้เสร็จสิ้น'
                    });
                    continue;
                }
            }
        ...
        res.status(200).json({ status: 'success', message: 'ดึงข้อมูลรายงานสำเร็จ', data: { results: finalResults } });
    } catch (err) { next(err); }
};
