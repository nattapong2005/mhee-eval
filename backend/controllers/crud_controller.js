...
exports.getOne = (modelName) => async (req, res, next) => {
    try {
        ...
        if (!doc) return next(new AppError('ไม่พบข้อมูลที่ระบุ', 404));
        res.status(200).json({ status: 'success', message: 'ดึงข้อมูลสำเร็จ', data: doc });
    } catch (err) { next(err); }
};
...
exports.create = (modelName) => async (req, res, next) => {
    try {
        ...
        res.status(201).json({ status: 'success', message: 'สร้างข้อมูลสำเร็จ', data: doc });
    } catch (err) { next(err); }
};
...
exports.update = (modelName) => async (req, res, next) => {
    try {
        ...
        res.status(200).json({ status: 'success', message: 'อัปเดตข้อมูลสำเร็จ', data: doc });
    } catch (err) { next(err); }
};
...
exports.delete = (modelName) => async (req, res, next) => {
    try {
        ...
        res.status(200).json({ status: 'success', message: 'ลบข้อมูลสำเร็จ', data: null });
    } catch (err) { next(err); }
};
