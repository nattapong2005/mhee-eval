...
export default function AdminEvaluationDetailsPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const handleAddAssignment = async () => {
        ...
        const evId = parseInt(assignmentForm.evaluatorId);
        const eeId = parseInt(assignmentForm.evaluateeId);

        if (evId === eeId) {
            Swal.fire('ข้อผิดพลาด', 'ไม่สามารถประเมินตัวเองได้', 'warning');
            return;
        }

        const isDuplicate = evaluation?.assignments?.some((a: any) =>
            (a.evaluator?.id === evId && a.evaluatee?.id === eeId) ||
            (a.evaluatorId === evId && a.evaluateeId === eeId)
        );

        if (isDuplicate) {
            Swal.fire('ข้อผิดพลาด', 'ผู้ประเมินและผู้รับการประเมินคู่นี้ถูกจับคู่ไปแล้ว', 'warning');
            return;
        }
                ...
                {tabs.map((tab) => {
                    ...
                    return (
                        <button
                            ...
                            className={`flex items-center gap-2.5 pb-4 pt-2 text-sm font-medium transition-all relative whitespace-nowrap ${active ? 'text-white' : 'text-gray-500 hover:text-gray-300'
                                }`}
                        >
                            <Icon size={18} />
                            {tab.name}
